...
export const rollGoldReward = (rule: CombatRewardRule): number => {
  const range = rule.gold.max - rule.gold.min + 1;
  return rule.gold.min + Math.floor(Math.random() * range);
};

export const createCombatCardRewards = (rule: CombatRewardRule): CardInstance[] => {
  const pool = CARD_DATABASE.filter(card => rule.cardRarities.includes(card.rarity));
  return shuffle(pool)
    .slice(0, rule.cardOptionCount)
    .map(card => ({ ...card, instanceId: generateId() }));
};

export const createRandomCardReward = (rarity: CardRarity, type?: CardType): CardInstance => {
  ...
  return { ...card, instanceId: generateId() };
};
