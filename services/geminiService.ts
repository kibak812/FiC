  ...
  async generateCard(context: CardGenerationContext, maxRetries: number = 3): Promise<GeneratedCard> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        // If invalid, adjust prompt for next attempt
        console.log(`Card generation attempt ${attempt + 1} failed validation, retrying...`);

      } catch (error) {
        console.error(`Card generation attempt ${attempt + 1} error:`, error);
...
export async function generateCardWithFallback(context: CardGenerationContext): Promise<GeneratedCard> {
  ...
  try {
    return await service.generateCard(context);
  } catch (error) {
    console.error('AI card generation failed, using fallback:', error);
    const card = getFallbackCard(context);
    return {
      card,
      validationResult: validateCardBalance(card),
